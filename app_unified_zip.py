...
<style>
    ...
    .metric-card {
        background: linear-gradient(135deg, #D91023 0%, #8B0000 100%);
        padding: 1rem;
        border-radius: 10px;
        color: white;
        text-align: center;
    ...
    .metric-card-blue {
        background: linear-gradient(135deg, #1f4e79 0%, #0d2b4e 100%);
        padding: 1rem;
        border-radius: 10px;
        color: white;
        text-align: center;
...
def extraer_csvs_de_zip(uploaded_zip):
    """Extrae todos los CSVs de un archivo ZIP subido"""
    csv_files = []
    try:
...
def extraer_csvs_de_rar(uploaded_rar):
    """Extrae todos los CSVs de un archivo RAR subido (requiere rarfile)"""
    csv_files = []
    try:
        import rarfile
        ...
        st.error("❌ Para archivos RAR necesitas instalar: `pip install rarfile`")
    except Exception as e:
        st.error(f"❌ Error leyendo RAR: {str(e)}")
    return csv_files

def obtener_csvs(uploaded_files, uploaded_zip, uploaded_rar):
    """Obtiene todos los CSVs de cualquier fuente (directo, ZIP o RAR)"""
    todos_csvs = []
...
def procesar_csvs(csv_list, tipo_eleccion):
    ...
    for csv_info in csv_list:
        ...
        try:
            ...
            if COL_PARTIDO not in df.columns or COL_VOTOS not in df.columns:
                archivos_error.append(f"⚠️ {nombre}: Columnas requeridas no encontradas")
                continue
            ...
            if tipo_eleccion == 'diputados':
                ...
                if region and region in ESCAÑOS_DIPUTADOS_2026:
                    ...
                    archivos_error.append(f"⚠️ {nombre}: Región no detectada")
                ...
                if 'UNICO' in nombre_norm or 'UNICO' in nombre_norm:
                    ...
                    votos_nacionales.append(df_valido)
                    archivos_ok.append(('DISTRITO ÚNICO', nombre, csv_info.get('source', 'individual')))
                else:
                    ...
                    if region:
                        ...
                        archivos_error.append(f"⚠️ {nombre}: Distrito no detectado")
    ...
    # Para Parlamento Andino, consolidar todo
...
with st.sidebar:
    ...
    if tipo_eleccion == 'diputados':
        ...
        • Método D'Hondt por región<br><br>
        <b>📦 Formato ZIP esperado:</b><br>
        <code>PR-ESP_Diputados_2026_AMAZONAS.csv</code><br>
        <code>PR-ESP_Diputados_2026_ANCASH.csv</code><br>
        ... (27 archivos)
        </div>
        """, unsafe_allow_html=True)
    elif tipo_eleccion == 'senado':
        ...
        <b>⚖️ SENADO</b><br>
        • 30 escaños<br>
        • Distrito Único: 30 escaños<br>
        • Distrito Múltiple: regiones específicas<br>
        • Valla: 5% nacional + 3 escaños<br><br>
        <b>📦 Formato ZIP esperado:</b><br>
        <code>...Distrito_Electoral_Único_...</code><br>
        <code>...Distrito_Electoral_Múltiple_LIMA.csv</code><br>
        <code>...Distrito_Electoral_Múltiple_RESIDENTES.csv</code>
        </div>
        """, unsafe_allow_html=True)
    else:
        st.markdown("""
        ...
        <code>ORGANIZACION POLÍTICA</code> y<br>
        <code>CANTIDAD DE VOTOS</code>
        </div>
        """, unsafe_allow_html=True)

    st.markdown("---")
    st.caption("v3.0 - Simulador Integral 2026 · ZIP/RAR Support")
...
if todos_csvs:
    ...
    for src, files in fuentes.items():
        with st.expander(f"📂 {src} ({len(files)} archivos)"):
            st.markdown("<div class='file-list'>" + "<br>".join([f"• {f}" for f in files]) + "</div>", unsafe_allow_html=True)
...
with col_btn2:
    procesar = st.button("🚀 PROCESAR ELECCIONES", type="primary", use_container_width=True, 
                        disabled=(len(todos_csvs) == 0))
...
if procesar and todos_csvs:
    ...
    with st.spinner(f"⏳ Procesando {tipo_eleccion.upper()}..."):
        ...
        # =====================================================
        # LÓGICA ESPECÍFICA POR ELECCIÓN
        # =====================================================
        ...
        if tipo_eleccion == 'diputados':
            ...
            cuadro['TOTAL REGIÓN'] = cuadro.sum(axis=1)
            cuadro.loc['TOTAL NACIONAL'] = cuadro.sum(numeric_only=True)

            total_escaños = int(cuadro.loc['TOTAL NACIONAL', 'TOTAL REGIÓN'])
            ...
            sim_u = calcular_dhondt(df_unico, ESCAÑOS_SENADO, COL_PARTIDO, COL_VOTOS)
            ...
            res_u = calcular_dhondt(df_unico[df_unico[COL_PARTIDO].isin(partidos_aptos)], ESCAÑOS_SENADO, COL_PARTIDO, COL_VOTOS)
            res_m = []
            ...
            fila_mult.index = ['DISTRITO MÚLTIPLE']
            fila_u = res_u.set_index('Partido')['Escaños'].to_frame().T
            fila_u.index = ['DISTRITO ÚNICO']
            ...
            cuadro.loc['TOTAL PARTIDO'] = cuadro.loc[['DISTRITO MÚLTIPLE', 'DISTRITO ÚNICO']].sum()
        ...
        if uploaded_excel and not df_candidatos.empty and tipo_eleccion == 'diputados':
            try:
                ...
                st.warning(f"⚠️ Error cruzando con Excel: {e}")
    ...
    if tipo_eleccion == 'diputados':
        ...
        with tab1:
            ...
            st.bar_chart(cuadro.drop('TOTAL NACIONAL').drop('TOTAL REGIÓN', axis=1), height=500)
        ...
        with tab1:
            st.subheader("Distribución de Escaños - Senado 2026")
            csv_data = cuadro.to_csv().encode('utf-8')
        ...
        with tab3:
            ...
            st.markdown(f"""
            **Método:** D'Hondt  
            **Distrito Único:** {ESCAÑOS_SENADO} escaños  
            **Distrito Múltiple:** Regiones específicas  
    ...
    <div class="info-box">
        ...
        <ul>
            ...
            <li><span class="election-badge badge-senado">⚖️ SENADO</span> 30 escaños · Distrito Único + Múltiple</li>
            <li><span class="election-badge badge-andino">🌎 PARLAMENTO ANDINO</span> 5 escaños · Circunscripción única</li>
        </ul>
        <p><b>📦 Novedad:</b> Ahora puedes subir tus archivos en <b>ZIP o RAR</b> para no tener que seleccionar CSVs uno por uno.</p>
